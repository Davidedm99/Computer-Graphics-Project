...
struct Texture {
	...
	void createCubicTextureImage();
	void createTextureImageView(VkImageViewType type, int i);
...
protected:
	...
	const char *TextureFile[6] = {"sky/right.jpg", "sky/left.jpg", "sky/top.jpg", "sky/bottom.jpg", "sky/front.jpg", "sky/back.jpg"};
	...
	void recreateSwapChain() {
		...
		//creation of the descriptor sets do also the relative uniform buffer
		descriptorSetsRecreation();
	...
	void cleanupSwapChain() {
		...
		//destroy skybox descriptor sets
		descriptorSetsCleanup();
	...
	void transitionImageLayout(VkImage image, VkFormat format,
		...
		//only this was in the original code
		//barrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		...
		//only those next 2 line were in the original code
	...
	virtual void objectsCleanup() = 0;
	virtual void pipelineCleanup() = 0;
	virtual void descriptorSetsCleanup() = 0;
	virtual void descriptorSetsRecreation() = 0;
    ...
    void cleanup() {
		...
		//tecnically here I have to clear only models, texture, descriptor sets layouts others are done in the cleanupSwapChain()
		objectsCleanup();
...
void Texture::createCubicTextureImage() {
	...
	for (int i = 0; i < 6; i++) {
		pixels[i] = stbi_load((TEXTURE_PATH + BP->TextureFile[i]).c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
		if (!pixels[i]) {
			std::cout << (TEXTURE_PATH + BP->TextureFile[i]).c_str() << "\n";
			throw std::runtime_error("failed to load texture image!");
		}
		std::cout << BP->TextureFile[i] << " -> size: " << texWidth
	...
	BP->createSkyBoxImage(texWidth, texHeight, mipLevels, textureImage, textureImageMemory);
...
void Texture::createTextureImage(std::string file) {
	...
	BP->copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight), 1);
	BP->generateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels, 1);

	vkDestroyBuffer(BP->device, stagingBuffer, nullptr);
	vkFreeMemory(BP->device, stagingBufferMemory, nullptr);
}

void Texture::createTextureImageView(VkImageViewType type, int i) {
...
void Texture::init(BaseProject *bp, std::string file) {
	...
	if (BP->isSkybox) {
		...
		createCubicTextureImage();
	}
	else {
		createTextureImage(file);
	}
	createTextureImageView(type, i);
...
void Pipeline::init(BaseProject *bp, const std::string& VertShader, const std::string& FragShader,
	...
	VkResult result = vkCreatePipelineLayout(BP->device, &pipelineLayoutInfo, nullptr, &pipelineLayout);
	if (result != VK_SUCCESS) {
	 	PrintVkError(result);
	...
	result = vkCreateGraphicsPipelines(BP->device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline);
	if (result != VK_SUCCESS) {
	 	PrintVkError(result);
